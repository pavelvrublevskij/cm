...
- **The Git tab's Push button stayed disabled on a branch with no upstream yet** — it gated on the count of commits ahead of the upstream, which a brand new local branch has none of because it has no upstream to diff against in the first place. Push now also enables whenever there's a remote to push to, and the server runs `git push -u origin <branch>` on that first push instead of a bare `git push`, which git refuses without an upstream.
- **A changed Markdown or HTML file opened on its diff instead of its preview** — a session's Files tab opens changed files with diff requested by default so non-renderable files land somewhere useful, but that request was overriding the Markdown/HTML preview default instead of losing to it. Preview now wins for previewable files regardless of how the file was opened; the diff is still one click away.
- **Markdown preview showed the file's YAML frontmatter as text** — a leading `---`/`---` metadata block (used by memory, skill and agent files) rendered as a literal horizontal-rule-wrapped paragraph instead of being treated as metadata. It's now stripped before rendering, everywhere markdown is previewed.
- **Links to a heading inside a markdown preview did nothing** — `marked` doesn't add `id` attributes to headings by default, so a link like `[Installation](#installation)` had nothing to jump to; worse, clicking it changed the page's URL fragment and the app's hash-based router tried to treat it as a navigation, leaving the preview entirely. Headings now get a GitHub-style slug id, and a same-page anchor click now scrolls to the matching heading instead of being handed to the router.

## v1.5.0

### Features

- **Project file structure and source editor in the session's Files tab** — the old "File Changes" tab is now **Files** and holds the whole project, not just the diff list. The files that session changed stay pinned at the top, rendered as a tree in the same row pattern as the project structure below it and carrying their new/edited/deleted badges; the full project tree (everything, lazily loaded folder by folder) sits underneath, with the changed files lifted out of it so nothing is listed twice. Folders collapse on click in both trees, sorting reorders each level, and a folder holding nothing but a single folder merges into one row (`src/main/java/app`) — computed server-side for the project tree, so it costs no extra requests. The tab is split: structure on the left, a source pane on the right, resizable by dragging the divider and collapsible by clicking it (share of the width and collapsed state are remembered).
- **Reading and editing any project file** — clicking a file, changed or not, opens it in the pane with **Source / Preview / Diff** buttons: Markdown and HTML land on Preview, everything else on Source, and Diff is offered for anything the session touched (deleted files go straight to the diff). Markdown previews render inline; HTML renders in a sandboxed iframe so a previewed page can't run scripts against CM. Preview reflects unsaved edits, not just what is on disk. Source is a CodeMirror editor with line numbers, a highlighted cursor line, bracket matching, in-editor search (Ctrl+F) and syntax highlighting for JS/TS, JSON, CSS, HTML, XML, Markdown, YAML, shell, Python, Java, Kotlin, Scala, C/C++, C#, Groovy/Gradle, SQL, Go, Rust, Swift, PHP, Ruby, Lua, PowerShell, Dockerfile, `.properties`/`.ini`/`.env`, TOML and patch files. Every theme now carries its own `--code-*` token palette, so highlighting follows the theme instead of CodeMirror's stock colours. Binary and very large (>1 MB) files are reported instead of opened. New endpoints: `GET /api/projects/:slug/files/tree`, `GET /api/projects/:slug/files/content`, `PUT /api/projects/:slug/files/content`.
- **Scratchpad tab rebuilt on the same layout** — the flat list and modal preview are gone; the Scratchpad tab now uses the same split layout as Files: file list on the left, the same pane on the right with Source / Preview, syntax highlighting and the ⋮ actions (open in editor, show in explorer, open folder). The pane is deliberately read-only — these are Claude's temporary working files — so it shows `read-only · size · modified` instead of save state. The shared parts live in a new `public/js/code-view.js` (`CodeView`): previewable-extension check, rendered preview, CodeMirror/textarea mount, and the resizable-collapsible split layout, all used by both tabs. Server-side, the duplicated `openPath`/`revealInFileManager` helpers moved to `lib/os-open.js` and the scratchpad route now shares `readFileForEditor` with its 200 KB preview cap passed in.
- **Session detail defaults to a 25 / 50 / 25 split** — file structure, source pane, and terminal. The terminal pane's default width dropped from 50% to 25% to make room; a width you have already dragged is still remembered.
- **Save state always visible** — an unsaved file is marked with ● on its row in the tree, next to the path in the pane header, and as "● Unsaved changes" in the pane's status bar, which also shows the current autosave mode and the last save time. Save with the button or Ctrl/Cmd+S. Switching to another file keeps the unsaved text in memory (and its ● marker) instead of dropping it, so edits survive browsing around; "Reload from disk" in the pane menu discards them deliberately.
- **Autosave, configurable in CM Settings** — a new **Editor** section under CM Settings turns autosave on or off (off by default) and sets the idle delay before an edit is written (default 2 s, minimum 0.5 s). Saves write straight to the file on disk — the same file Claude may be editing in that session — with no backup copy and last-write-wins.
- **Line endings are preserved on save** — a file that uses CRLF is written back with CRLF, so saving a Windows file no longer rewrites every line.
- **Syntax colours in diffs** — diff lines are tokenized with the file's CodeMirror mode and share the editor's `--code-*` palette, so a diff reads like the source it came from. Added/removed line backgrounds are unchanged. Each line is tokenized on its own, so a construct spanning lines (a block comment, a template literal) colours per line rather than per file.
- **Diff shows only the diff** — `.md` files used to open the diff view on a rendered Markdown preview with a Preview/Diff sub-toggle inside it. Preview is now a first-class mode in the pane header, so the diff view is just the diff. The `/diff-current` response no longer carries the full `currentText` copy that the embedded preview needed.
- **Diff window still available** — the full-screen resizable diff modal moved to "Open diff in window" in the pane header's ⋮ menu, still with prev/next navigation, now stepping through every changed file that has a diff to show.

### Bug fixes

- **Edited files disappeared from a session's file list** — newer Claude Code versions still write file backups into `~/.claude/file-history/<session>/`, but no longer record `file-history-snapshot` entries in the session transcript, which is where CM read the file → backup mapping from. Without it, edited files came back with no hash and no versions, and a filter that assumed "no snapshot means nothing to show" then dropped them, so a session listed only the files it had *created*. CM now recomputes the mapping — backups are named `sha256(<absolute path>)` truncated to 16 hex, plus `@v<n>` — so edited files reappear with working diffs. Files whose backups genuinely are missing are still listed (source only, no Diff toggle), and sessions that do carry snapshot records keep using them unchanged.
- **Closing an active session from the footer bar only cleared the footer** — the × called `/deactivate` and re-rendered the bar and sidebar, but nothing else was told about it: the project session list kept its cached green "active" dot, the dashboard kept the session in its Active sessions block, and the session's own detail screen stayed open and live-polling as if nothing happened, so the close only became visible after navigating back to the project session list. Closing a session now clears the cached active flags and rendered dots for it, drops it from the dashboard's active list, and — when it is the session currently being viewed — stops the auto-refresh, closes its browser terminal pane and returns to the project session list.
- **Markdown Preview/Diff toggle broke outside the diff modal** — `_showMdPane` looked its buttons up through the modal's `#fh-diff-body` wrapper, so the toggle threw when the same diff rendered anywhere else; it now resolves them relative to the preview itself.
- **Files changed via a Bash command never showed up in a session's Files tab** — the changed-files list only scanned the transcript for `Write`/`Edit`/`MultiEdit`/`NotebookEdit` tool calls, so a file a session modified by running a shell command (e.g. `cp` from a scratchpad build step) was invisible even though it was genuinely changed. When the project is a git repo, `/context` now also picks up locally-changed files reported by `git status` whose mtime falls inside the session's time window.
