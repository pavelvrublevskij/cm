...
- **Untracked directories collapsed to a single entry in the Git tab** — every file in a new folder is now listed individually, and the changes list defaults to Tree view.
- **A broken node-pty install left the terminal panel silently unusable** — startup now verifies and repairs the pty binding, and a spawn failure shows a "Fix & Restart App" banner.
- **Files changed via a Bash command could be wrongly attributed to a session** — the mtime-based git-status fallback is removed; the Files tab now relies solely on recorded tool calls.
- **The Git tab's Push button stayed disabled on a branch with no upstream yet** — Push now also enables when a remote exists, and uses `git push -u` on the first push.
- **A changed Markdown or HTML file opened on its diff instead of its preview** — preview now wins for previewable files regardless of how the file was opened.
- **Markdown preview showed the file's YAML frontmatter as text** — frontmatter is now stripped before rendering.
- **Links to a heading inside a markdown preview did nothing** — headings now get a GitHub-style slug id, and same-page anchor clicks scroll instead of being routed away.
- **Background subagent token usage was invisible in usage stats** — a subagent's resolved model and token count are now picked up and folded into per-model usage totals instead of being dropped.

## v1.5.0

### Features

- **Project file structure and source editor in the session's Files tab** — File Changes is now Files and holds the whole project, changed files pinned at the top, split into a tree on the left and a source pane on the right.
- **Reading and editing any project file** — click any file for Source / Preview / Diff, with a CodeMirror editor and syntax highlighting across most common languages.
- **Scratchpad tab rebuilt on the same layout** — the flat list and modal preview are gone; it now uses the same split file-list/pane layout as Files, read-only.
- **Session detail defaults to a 25 / 50 / 25 split** — file structure, source pane, and terminal, with the terminal's default width now 25%.
- **Save state always visible** — an unsaved file is marked with ● everywhere it appears; save with the button or Ctrl/Cmd+S, edits survive switching files.
- **Autosave, configurable in CM Settings** — a new Editor section toggles autosave and its idle delay (off by default).
- **Line endings are preserved on save** — a CRLF file is written back with CRLF instead of being rewritten line by line.
- **Syntax colours in diffs** — diff lines are tokenized and colored like the source they came from.
- **Diff shows only the diff** — Markdown/HTML preview is now a first-class pane mode, so the diff view no longer doubles as a preview.
- **Diff window still available** — moved to "Open diff in window" in the pane's ⋮ menu, with prev/next navigation across changed files.

### Bug fixes

- **Edited files disappeared from a session's file list** — newer Claude Code versions stopped recording the snapshot entries CM used to map files to backups; the mapping is now recomputed instead.
- **Closing an active session from the footer bar only cleared the footer** — closing now also clears cached active state everywhere and, if you're viewing that session, backs you out of it.
- **Markdown Preview/Diff toggle broke outside the diff modal** — its buttons were looked up through the modal's wrapper; they're now resolved relative to the preview itself.
- **Files changed via a Bash command never showed up in a session's Files tab** — `git status` is now also checked (for git projects) alongside the recorded tool calls.
