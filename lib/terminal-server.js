 ...
 * fix, and each has a one-line manual workaround worth surfacing. spawn-helper is a macOS-only build
 * target (see node-pty's binding.gyp — only built under 'OS=="mac"'); Linux's pty.cc calls forkpty()
 * directly, so there is nothing to inspect there even on a perfectly good install.
 */
function diagnoseSpawnFailure() {
  if (process.platform === 'win32' || !pty) return null;
  if (process.platform === 'darwin' && nodePtyRoot()) {
  ...
  // Nothing platform-specific left to inspect — the remaining common cause is exhausted pty/fd
  // slots from long-running background terminals (each stays open until its process exits or is
  // closed). Restarting the app (Fix & Restart App) releases every fd it holds, which clears this
  // case too.
  return 'This may be exhausted pty slots from background terminals. Fix & Restart App will release them.';
}
