...
/** git quotes paths with special chars/spaces (unless core.quotepath=false) — strip and unescape. */
function unquotePath(file) {
...
function parseStatus(porcelain) {
  return porcelain.split(/\r?\n/).filter(Boolean).map(line => {
    ...
    let file = unquotePath(line.slice(3));
...
/**
 * Added/removed line counts for every tracked file that differs from HEAD — the same working-tree-
 * vs-HEAD comparison the per-file diff endpoint shows, so the numbers here always match what
 * clicking through to the diff would report. Renames are turned off: a renamed file then reads as a
 * plain add under its new path, matching the diff endpoint's own view (it can't find the new path in
 * HEAD either, so it reads the whole file as added). Untracked files don't appear here — they have no
 * HEAD side for git to diff against — and binary files report as `-`/`-`, mapped to `binary: true`.
 */
async function diffNumstat(cwd) {
  let raw;
  try { raw = await git(['diff', 'HEAD', '--numstat', '--no-renames'], cwd); }
  catch (_) { return {}; }
  if (!raw) return {};
  ...
  for (const line of raw.split(/\r?\n/)) {
    ...
    stats[path] = (added === '-' || removed === '-')
      ? { binary: true }
      : { added: parseInt(added, 10), removed: parseInt(removed, 10) };
  }
  return stats;
}
