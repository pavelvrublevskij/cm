...
function resolveModel(model, pricing) {
  if (!model || model === '<synthetic>') return null;
  if (resolveModelPrice(model, pricing || getCurrentPricing())) return model;
  return null;
}

function getPricing(model, pricing) {
  return resolveModelPrice(model, pricing || getCurrentPricing()) || FALLBACK_PRICING['claude-sonnet-4-6'];
}
...
function calcCost(tokens, model, pricing) {
  const r = getPricing(model, pricing);
...
function calcCostMultiModel(byModel, pricing) {
  pricing = pricing || getCurrentPricing();
  let total = { input: 0, output: 0, cache_write: 0, cache_read: 0, total: 0 };
  for (const [model, tokens] of Object.entries(byModel)) {
    const c = calcCost(tokens, model, pricing);
    total.input += c.input;
...
function parseSessionUsage(filePath, pricing) {
  pricing = pricing || getCurrentPricing();
  const byModel = {};
  const daily = {};
  const hourly = {};
  ...
  for (const line of lines) {
    try {
      ...
      if (entry.type === 'assistant' && entry.message?.usage) {
        ...
        const model = resolveModel(entry.message.model, pricing);
        if (!model) continue;
  ...
  for (const [agentId, notification] of Object.entries(agentNotifications)) {
    const model = resolveModel(agentModels[agentId], pricing);
    if (!model) continue;
...
function buildIndex() {
  ...
  // Fetched once per reindex pass (and only if at least one session actually needs reparsing),
  // instead of once per session, since parseSessionUsage() would otherwise hit disk for it again
  // on every stale session file.
  let pricing = null;
  ...
  for (const d of dirs) {
    ...
    for (const f of files) {
      ...
      if (!pricing) pricing = getCurrentPricing();
      const { totals, byModel, daily, hourly } = parseSessionUsage(filePath, pricing);
...
module.exports = { getModelPricingMap, calcCost, calcCostMultiModel, addTokens, emptyTokens, buildIndex, getSessionUsage, getProjectUsageMap, removeSessionFromIndex, parseSessionUsage };
